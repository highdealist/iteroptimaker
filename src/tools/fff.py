...
class NotebookFileConverter:
    ...
    def __init__(self):
        ...
        self.toggle_btn = ttk.Button(self.toggle_frame, text="Toggle Color Mode", command=self.toggle_color_mode)
    ...
    def setup_logging(self):
        logging.basicConfig(
            ...
            handlers=[logging.StreamHandler(), logging.FileHandler('notebook_converter.log')]
        )

    def setup_converter_tab(self):
        mode_frame = ttk.LabelFrame(self.converter_frame, text="Conversion Mode", padding="10", style='Section.TLabelframe')
        mode_frame.grid(row=0, column=0, columnspan=3, sticky="ew", pady=(0, 10))
        self.mode_var = tk.StringVar(value="files_to_notebook")
        files_to_nb = ttk.Radiobutton(mode_frame, text="Files to Notebook", variable=self.mode_var, value="files_to_notebook", command=self.update_ui)
        files_to_nb.grid(row=0, column=0, padx=20)
        nb_to_files = ttk.Radiobutton(mode_frame, text="Notebook to Files", variable=self.mode_var, value="notebook_to_files", command=self.update_ui)
        nb_to_files.grid(row=0, column=1, padx=20)
        self.create_tooltip(files_to_nb, "Combine multiple source files into a single Jupyter notebook")
        self.create_tooltip(nb_to_files, "Extract code cells from a Jupyter notebook into separate files")
    ...
    def setup_sources_frame(self):
        ...
        browse_file_btn = ttk.Button(browse_frame, text="Browse Files", command=self.browse_multiple_files)
        browse_file_btn.pack(side=tk.LEFT, padx=2)
        self.create_tooltip(browse_file_btn, "Select multiple files to include")
        browse_dir_btn = ttk.Button(browse_frame, text="Browse Folders", command=self.browse_multiple_directories)
        ...
        subdir_check = ttk.Checkbutton(input_frame, text="Include Subdirectories", variable=self.include_subdirs_var)
        ...
        help_btn = ttk.Button(exclude_frame, text="?", width=3, command=self.show_pattern_help)
        ...
        format_combo = ttk.Combobox(output_frame, textvariable=self.output_format_var, values=["ipynb", "py", "html", "md", "pdf"])
        ...
        browse_output_btn = ttk.Button(output_frame, text="Browse", command=lambda: self.browse_path(self.output_dir_var))
        ...
        ttk.Button(self.sources_frame, text="Run Conversion", command=self.run_conversion).grid(row=4, column=0, columnspan=3, pady=10)
    ...
    def setup_notebook_frame(self):
        ...
        browse_nb_btn = ttk.Button(self.notebook_frame, text="Browse", command=self.browse_notebook)
        ...
        browse_output_btn = ttk.Button(self.notebook_frame, text="Browse", command=lambda: self.browse_path(self.nb_output_dir_var))
        browse_output_btn.grid(row=1, column=2)
        ttk.Button(self.notebook_frame, text="Extract Files", command=self.extract_files).grid(row=2, column=0, columnspan=3, pady=10)

    def browse_notebook(self):
        filename = filedialog.askopenfilename(title="Select Notebook", filetypes=[("Jupyter Notebooks", "*.ipynb"), ("All Files", "*.*")])
        if filename:
    ...
    def setup_directory_tab(self):
        ...
        browse_btn = ttk.Button(self.directory_frame, text="Browse", command=lambda: self.browse_path(self.dir_path_var))
        ...
        update_btn = ttk.Button(button_frame, text="Update Structure", command=self.update_directory_structure)
        update_btn.pack(side=tk.LEFT, padx=5)
        save_btn = ttk.Button(button_frame, text="Save Structure", command=self.save_directory_structure)
    ...
    def browse_multiple_directories(self):
        ...
        ttk.Button(select_window, text="Add Directory", command=select_dir).pack(pady=10)
        ttk.Button(select_window, text="Done", command=select_window.destroy).pack(pady=5)
